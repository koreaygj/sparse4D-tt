...
from models.experimental.retinanet.tt.tt_bottleneck import (
    TTBottleneck,
...
def load_input(name):
    ...
    img = Image.open("models/experimental/retinanet/resources/dog_800x800.jpg").convert(
        "RGB"
    )
...
class BottleneckTestInfra:
    def __init__(
        ...
        self.name = name
        self.inputs_mesh_mapper, self.weights_mesh_mapper, self.output_mesh_composer = (
            self.get_mesh_mappers(device)
        )
    ...
    def run(self):
        self.output_tensor, _ = self.ttnn_model(
            self.input_tensor, self.device, self.input_tensor.shape
        )
        return self.output_tensor

    def validate(self, output_tensor=None):
        tt_output_tensor = (
            self.output_tensor if output_tensor is None else output_tensor
        )
...
def test_bottleneck(
    device, batch_size, channels, height, width, stride, dilation, downsample, name
):
    BottleneckTestInfra(
        device,
        batch_size,
